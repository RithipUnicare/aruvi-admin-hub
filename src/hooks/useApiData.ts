...
export function useApiData() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `Fetching data for c_no: ${params.c_no}, type: ${params.type}`
      );

      // Fetch categories first
      const categoriesResponse = await fetch(
        "https://deepikagroups.in/admin/api/get_categories.php"
      );
      const categoriesData = await categoriesResponse.json();
      ...
      // Fetch all products at once using the new endpoint
      const productsResponse = await fetch(
        `https://deepikagroups.in/admin/api/allProducts.php?company_no=${params.c_no}&type=${params.type}`
      );
      const productsData = await productsResponse.json();
      const allProducts: Product[] = [];

      if (productsData.Product) {
        productsData.Product.forEach((prod: any) => {
          // Determine price based on type
          let price = DEFAULT_PRICE;
          if (params.type === "Godown") {
            price = parseFloat(prod.counter2_price || "0");
          } else {
            price = parseFloat(prod.c_price || "0");
          }
      ...
      console.log(
        `Data loaded successfully: ${fetchedCategories.length} categories, ${allProducts.length} products`
      );
    } catch (error) {
      console.error("Failed to fetch data:", error);
      toast({
